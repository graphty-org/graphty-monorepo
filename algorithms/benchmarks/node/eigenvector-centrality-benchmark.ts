...
// Node.js Eigenvector Centrality Performance Benchmark using Benchmark.js
import { CrossPlatformBenchmark } from '../utils/benchmark-runner'
import { generateTestGraphs } from '../utils/test-data-generator'
import { convertToLibraryGraph } from '../utils/graph-adapter'
import { eigenvectorCentrality } from '../../src/algorithms/centrality/eigenvector'
import { saveBenchmarkSession } from '../utils/benchmark-result'
import { formatSystemInfo, getSystemInfo } from '../utils/system-info'

// Configuration for Node.js benchmarks
// Eigenvector Centrality complexity: O(V*E*iterations)
const configs = {
  quick: {
...
async function runEigenvectorCentralityBenchmark(configType: 'quick' | 'comprehensive') {
  console.log(`🚀 Running ${configType} Eigenvector Centrality benchmarks in Node.js`)
  ...
  const benchmark = new CrossPlatformBenchmark(config, `Eigenvector Centrality ${configType} Performance`)
  ...
  for (const [key, testData] of testGraphs.entries()) {
    benchmark.addTest(
      `Eigenvector Centrality ${testData.graphSize} vertices (${testData.graphType})`,
      () => {
        const result = eigenvectorCentrality(testData.graph)
        // Verify result to prevent dead code elimination
        if (!result) { throw new Error('Eigenvector Centrality returned empty result') }
      },
      testData,
      {
...
async function main() {
  ...
  try {
    await runEigenvectorCentralityBenchmark(configType)
  } catch (error) {
