...
async function runHITSBenchmark(configType: "quick" | "comprehensive") {
    ...
    const benchmark = new CrossPlatformBenchmark(config, `HITS ${configType} Performance`);
    ...
    for (const [key, testData] of testGraphs.entries()) {
        benchmark.addTest(
            ...
            () => {
                ...
                if (!result) {
                    throw new Error("HITS returned empty result");
                }
            },
            testData,
            {
...
async function main() {
    ...
    try {
        await runHITSBenchmark(configType);
    } catch (error) {
