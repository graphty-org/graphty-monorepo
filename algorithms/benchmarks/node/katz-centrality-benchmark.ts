...
// Node.js Katz Centrality Performance Benchmark using Benchmark.js
import { CrossPlatformBenchmark } from "../utils/benchmark-runner";
import { generateTestGraphs } from "../utils/test-data-generator";
import { convertToLibraryGraph } from "../utils/graph-adapter";
import { katzCentrality } from "../../src/algorithms/centrality/katz";
import { saveBenchmarkSession } from "../utils/benchmark-result";
import { formatSystemInfo, getSystemInfo } from "../utils/system-info";

// Configuration for Node.js benchmarks
// Katz Centrality complexity: O(V*E*iterations)
const configs = {
    quick: {
...
async function runKatzCentralityBenchmark(configType: "quick" | "comprehensive") {
    console.log(`🚀 Running ${configType} Katz Centrality benchmarks in Node.js`);
    console.log("=" + "=".repeat(50));
    console.log(formatSystemInfo(getSystemInfo()));
    console.log("");

    const config = configs[configType];
    const benchmark = new CrossPlatformBenchmark(config, `Katz Centrality ${configType} Performance`);
    ...
    for (const [key, testData] of testGraphs.entries()) {
        benchmark.addTest(
            `Katz Centrality ${testData.graphSize} vertices (${testData.graphType})`,
            () => {
                const result = katzCentrality(testData.graph, { alpha: 0.1, beta: 1.0 });
                // Verify result to prevent dead code elimination
                if (!result || Object.keys(result).length === 0) {
                    throw new Error("Katz Centrality returned empty result");
                }
            },
            testData,
            {
...
async function main() {
    ...
    try {
        await runKatzCentralityBenchmark(configType);
    } catch (error) {
