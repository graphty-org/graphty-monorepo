...
export async function runDegreeCentrality(graphData) {
    ...
    console.log(
        `Added ${graphData.nodes.length} nodes:`,
    ...
    console.log(
        `Added ${graphData.edges.length} edges:`,
    ...
    const sortedNodes = Object.entries(packageResult).sort(([, a], [, b]) => b - a);
...
export async function runDegreeCentralityEducational(graphData) {
    ...
    console.log(
        `Added ${graphData.nodes.length} people:`,
        graphData.nodes.map((n) => n.id),
    );
    ...
    console.log(
        `Added ${graphData.edges.length} friendships:`,
