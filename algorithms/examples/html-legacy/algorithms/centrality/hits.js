...
export function runHITS() {
    ...
    const hubEntries = Object.entries(result.hubs).sort(([, a], [, b]) => b - a);
    hubEntries.forEach(([node, score]) => {
        console.log(`  ${node}: ${score.toFixed(4)}`);
    });

    console.log("\nAuthority Scores:");
    const authEntries = Object.entries(result.authorities).sort(([, a], [, b]) => b - a);
    ...
    combined.forEach(({ node, hub, auth, combined }, index) => {
        console.log(
            `  ${index + 1}. ${node}: Hub=${hub.toFixed(3)}, Auth=${auth.toFixed(3)}, Total=${combined.toFixed(3)}`,
        );
    });
    ...
    nodes.forEach((node) => {
        ...
        console.log(
            `${node}: out=${outDegree}, in=${inDegree}, hub=${hubScore.toFixed(3)}, auth=${authScore.toFixed(3)}`,
        );
    });
