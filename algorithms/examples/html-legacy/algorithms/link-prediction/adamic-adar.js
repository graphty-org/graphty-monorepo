...
export function runAdamicAdar() {
    ...
    predictions.slice(0, 3).forEach((pred) => {
        ...
        console.log(
            `  Average weight per neighbor: ${commonNeighbors.length > 0 ? (totalWeight / commonNeighbors.length).toFixed(3) : 0}`,
        );
...
export function demonstrateApplications() {
    ...
    userTypes.forEach((user) => {
        ...
        console.log(`\n${user.name} (${user.type}):`);
        console.log(
            `  Top recommendations: ${personalPredictions
                .slice(0, 2)
                .map((p) => (p.source === user.name ? p.target : p.source))
                .join(", ")}`,
        );
...
export function performanceAnalysis() {
    ...
    const cnPredictions = people
        .flatMap((person1) =>
            people
                .filter((person2) => person1 < person2 && !graph.hasEdge(person1, person2))
                .map((person2) => ({
                    source: person1,
                    target: person2,
