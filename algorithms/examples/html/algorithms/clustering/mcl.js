...
export function runMarkovClustering() {
    ...
    expansionValues.forEach((expansion) => {
        ...
        inflationValues.forEach((inflation) => {
            ...
            console.log(
                `Attractor ratio: ${attractorRatio.toFixed(2)} (${result.attractors.size}/${result.communities.length})`,
            );
        });
    });
...
export function demonstrateParameterEffects() {
    ...
    [1.5, 2.0, 3.0].forEach((inflation) => {
        ...
        console.log(
            `   Inflation ${inflation}: ${result.communities.length} clusters, ${result.attractors.size} attractors`,
        );
    });
    ...
    [2, 3].forEach((expansion) => {
        ...
        console.log(
            `   Expansion ${expansion}: ${result.communities.length} clusters, ${result.attractors.size} attractors`,
        );
    });
}
