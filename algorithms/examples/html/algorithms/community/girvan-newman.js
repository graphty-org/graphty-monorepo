...
export function testAlgorithmParameters() {
    ...
    console.log(
        `With maxCommunities=2: Found ${limitedResult[limitedResult.length - 1].communities.length} communities`,
    );
...
export function testEdgeBetweennessAnalysis() {
    ...
    dendrogram.forEach((step, index) => {
        if (index === 0) {
            ...
            console.log(
                `Step ${index + 1}: ${step.communities.length} communities, Modularity: ${step.modularity.toFixed(4)}`,
            );
        }
    });
