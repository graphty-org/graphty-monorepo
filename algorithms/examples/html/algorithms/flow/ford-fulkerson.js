...
export function runFordFulkerson() {
    ...
    console.log(
        "Flow graph:",
        Array.from(result.flowGraph.entries()).map(([from, flows]) => ({
            from,
    ...
    console.log(
        "Flow utilization:",
        ((result.maxFlow / Math.min(totalCapacityFromSource, totalCapacityToSink)) * 100).toFixed(1) + "%",
    );
