...
export class UIControls {
    ...
    addSlider(groupId, id, options = {}) {
        const { min = 0, max = 100, value = 50, step = 1, callback = null } = options;
    ...
    addSelect(groupId, id, options = {}) {
        const { choices = [], value = null, callback = null } = options;
    ...
    addNumberInput(groupId, id, options = {}) {
        const { min = 1, max = 100, value = 10, callback = null } = options;
    ...
    addButton(groupId, id, options = {}) {
        const { text = "Button", className = "button", callback = null } = options;
...
export class ResultsDisplay {
    ...
    formatValue(value) {
        ...
        if (typeof value === "number") {
            return value === Infinity ? "∞" : value % 1 === 0 ? value.toString() : value.toFixed(2);
        }
