...
for (const [node, coreness] of flowCores.coreness) {
    const role = node.startsWith("News") ? "Source" : node.startsWith("Blog") ? "Amplifier" : "Consumer";
...
const timeSteps = [
    ...
    [
        ...
        ["D", "B"],
        ["D", "C"],
    ], // Time 2: Add central node
    [
        ["E", "D"],
        ["E", "A"],
    ], // Time 3: Add peripheral node
    [
        ["F", "E"],
        ["F", "D"],
        ["F", "B"],
    ], // Time 4: More connections
];
...
for (let t = 0; t < timeSteps.length; t++) {
    ...
    console.log(
        `  Nodes: ${growingNetwork.size}, Edges: ${Array.from(growingNetwork.values()).reduce((sum, neighbors) => sum + neighbors.size, 0) / 2}`,
    );
}
