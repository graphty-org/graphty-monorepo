...
inflationValues.forEach((inflation) => {
    ...
    console.log(
        `${inflation.toFixed(1).padEnd(9)} | ${result.communities.length.toString().padEnd(8)} | ${result.converged.toString().padEnd(9)} | ${desc} clustering`,
    );
});
...
// Verify results
console.log("\n=== Verification ===");
console.log("✓ Found 3 main protein complexes:", mclDefault.communities.length >= 3);
console.log("✓ All nodes assigned to clusters:", mclDefault.communities.flat().length === proteinNetwork.nodeCount);
console.log(
    "✓ Higher inflation creates more clusters:",
...
console.log("✓ Algorithm converged:", mclDefault.converged);
console.log("✓ Algorithm converges:", mclDefault.iterations < 100);
