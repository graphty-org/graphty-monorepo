...
async function copyFileWithWarning(src, dest, sourceFile) {
    let content = await fs.readFile(src, "utf-8");

    // Add auto-generation warning based on file type
    const ext = path.extname(dest).toLowerCase();
    let warning = "";

    if (ext === ".html") {
        warning = `<!-- DO NOT EDIT: THIS FILE IS AUTOMATICALLY GENERATED. EDIT ${sourceFile} INSTEAD. -->\n`;
    } else if (ext === ".css") {
        warning = `/* DO NOT EDIT: THIS FILE IS AUTOMATICALLY GENERATED. EDIT ${sourceFile} INSTEAD. */\n`;
    } else if (ext === ".js") {
        warning = `// DO NOT EDIT: THIS FILE IS AUTOMATICALLY GENERATED. EDIT ${sourceFile} INSTEAD.\n`;
    ...
    await fs.writeFile(dest, warningContent);
}
...
async function processExampleHtml(htmlPath, outputPath) {
    ...
    const sourceFile = path.relative(path.resolve(__dirname, ".."), htmlPath);
    const warning = `<!-- DO NOT EDIT: THIS FILE IS AUTOMATICALLY GENERATED. EDIT ${sourceFile} INSTEAD. -->\n`;

    // The examples already use "./algorithms.js" which will work with our copied bundle
    // No changes needed to the imports

    await fs.writeFile(outputPath, warning + content);
}

async function copyDirectory(src, dest, addWarnings = false) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            await copyDirectory(srcPath, destPath, addWarnings);
        } else {
            if (addWarnings && (entry.name.endsWith(".css") || entry.name.endsWith(".html"))) {
                const sourceFile = path.relative(path.resolve(__dirname, ".."), srcPath);
                await copyFileWithWarning(srcPath, destPath, sourceFile);
            } else {
...
async function buildGitHubPages() {
    ...
    try {
        ...
        const sharedRootSrcDir = path.join(rootDir, "gh-pages-src", "gh-pages-assets", "shared");
        const sharedRootDestDir = path.join(ghPagesDir, "shared");
        await copyDirectory(sharedRootSrcDir, sharedRootDestDir, true);
        console.log("Copied shared/ directory with design-system.css");

        // 4. Copy main landing page
        const mainIndexSrc = path.join(rootDir, "gh-pages-src", "gh-pages-index.html");
        const mainIndexDest = path.join(ghPagesDir, "index.html");
        await copyFileWithWarning(mainIndexSrc, mainIndexDest, "gh-pages-src/gh-pages-index.html");
...
<body>
    ...
    <footer class="footer">
        ...
        // Copy every example page and its script (the examples index links to all of them)
        for (const file of await fs.readdir(shortestPathSrcDir)) {
        ...
        // Copy every example page and its script (the examples index links to all of them)
        for (const file of await fs.readdir(centralitySrcDir)) {
