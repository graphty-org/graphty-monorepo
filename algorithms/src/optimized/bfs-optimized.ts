import type { NodeId, TraversalResult, ShortestPathResult, TraversalOptions } from "../types/index.js";
import type { Graph } from "../core/graph.js";
import { CSRGraph } from "./csr-graph.js";
import { DirectionOptimizedBFS } from "./direction-optimized-bfs.js";
import { toCSRGraph, getOptimizationConfig } from "./graph-adapter.js";
...
const csrCache = new WeakMap<Graph, CSRGraph<NodeId>>();
...
export function bfsOptimized(
  ...
  const bfsOptions: any = {};
  if (options.alpha !== undefined || config.bfsAlpha !== undefined) {
    bfsOptions.alpha = options.alpha ?? config.bfsAlpha;
  }
  if (options.beta !== undefined || config.bfsBeta !== undefined) {
    bfsOptions.beta = options.beta ?? config.bfsBeta;
  }
