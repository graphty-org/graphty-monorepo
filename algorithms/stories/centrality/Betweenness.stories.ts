/**
 * Betweenness Centrality Algorithm Stories
 *
 * Demonstrates betweenness centrality with a before/after heat map visualization.
 * Shows how node importance is determined by how often it lies on shortest paths.
 *
 * IMPORTANT: This story uses the actual betweennessCentrality implementation
...
function createBetweennessStory(args: BetweennessArgs): HTMLElement {
    ...
    // Create legend
    const legend = createHeatMapLegend("Betweenness Centrality", "Low", "High");
    ...
    scoresPanel.innerHTML = `
        <div style="font-weight: 600; margin-bottom: 8px; color: #1e293b;">Betweenness Centrality Scores</div>
    ...
    updateStatus(statusPanel, "Click 'Calculate' to compute betweenness centrality");
    ...
    function apply(): void {
        ...
        updateScoresDisplay();
        updateStatus(statusPanel, "Betweenness centrality computed! Nodes on many shortest paths are larger/redder.");
    }
...
const meta: Meta<BetweennessArgs> = {
    title: "Centrality",
    argTypes: {
        nodeCount: {
...
type Story = StoryObj<BetweennessArgs>;

/**
 * Betweenness centrality story with before/after visualization.
 *
 * This story uses the actual `betweennessCentrality()` function from @graphty/algorithms.
 * Nodes are colored and sized based on how often they appear on shortest paths.
 */
export const Betweenness: Story = {
    render: (args) => createBetweennessStory(args),
