/**
 * Closeness Centrality Algorithm Stories
 *
 * Demonstrates closeness centrality with a before/after heat map visualization.
 * Shows how node importance is determined by average distance to all other nodes.
 *
 * IMPORTANT: This story uses the actual closenessCentrality implementation
...
function createClosenessStory(args: ClosenessArgs): HTMLElement {
    const { nodeCount, graphType, normalized, harmonic, seed } = args;
    ...
    // Create legend
    const legend = createHeatMapLegend(harmonic ? "Harmonic Centrality" : "Closeness Centrality", "Low", "High");
    ...
    scoresPanel.innerHTML = `
        <div style="font-weight: 600; margin-bottom: 8px; color: #1e293b;">${harmonic ? "Harmonic" : "Closeness"} Centrality Scores</div>
    ...
    updateStatus(statusPanel, "Click 'Calculate' to compute closeness centrality");
    ...
    function apply(): void {
        ...
        updateScoresDisplay();
        updateStatus(statusPanel, "Closeness centrality computed! Central nodes can reach others quickly.");
    }
...
const meta: Meta<ClosenessArgs> = {
    ...
    argTypes: {
        ...
        harmonic: {
            control: { type: "boolean" },
            description: "Use harmonic mean (better for disconnected graphs)",
        },
        seed: {
            control: { type: "number" },
...
type Story = StoryObj<ClosenessArgs>;

/**
 * Closeness centrality story with before/after visualization.
 *
 * This story uses the actual `closenessCentrality()` function from @graphty/algorithms.
 * Nodes are colored and sized based on their average distance to all other nodes.
 */
export const Closeness: Story = {
    render: (args) => createClosenessStory(args),
