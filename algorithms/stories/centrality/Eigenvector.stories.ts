/**
 * Eigenvector Centrality Algorithm Stories
 *
 * Demonstrates eigenvector centrality with a before/after heat map visualization.
 * Shows how node importance is determined by the importance of its neighbors.
 *
 * IMPORTANT: This story uses the actual eigenvectorCentrality implementation
...
function createEigenvectorStory(args: EigenvectorArgs): HTMLElement {
    ...
    // Create legend
    const legend = createHeatMapLegend("Eigenvector Centrality", "Low", "High");
    ...
    scoresPanel.innerHTML = `
        <div style="font-weight: 600; margin-bottom: 8px; color: #1e293b;">Eigenvector Centrality Scores</div>
    ...
    updateStatus(statusPanel, "Click 'Calculate' to compute eigenvector centrality");
    ...
    function apply(): void {
        ...
        updateScoresDisplay();
        updateStatus(statusPanel, "Eigenvector centrality computed! Well-connected to well-connected nodes.");
    }
...
const meta: Meta<EigenvectorArgs> = {
    title: "Centrality",
    argTypes: {
        nodeCount: {
...
type Story = StoryObj<EigenvectorArgs>;

/**
 * Eigenvector centrality story with before/after visualization.
 *
 * This story uses the actual `eigenvectorCentrality()` function from @graphty/algorithms.
 * Nodes are colored and sized based on the importance of their neighbors.
 */
export const Eigenvector: Story = {
    render: (args) => createEigenvectorStory(args),
