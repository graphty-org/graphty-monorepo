/**
 * Hierarchical Clustering Algorithm Stories
 *
 * Demonstrates hierarchical clustering with a before/after visualization.
 * Shows how nodes are colored by their cluster membership.
 *
 * IMPORTANT: This story uses the actual hierarchicalClustering implementation
...
/**
 * Extract clusters from hierarchical clustering result at the closest level
...
/**
 * Create the Hierarchical Clustering visualization story.
 */
function createHierarchicalStory(args: HierarchicalArgs): HTMLElement {
    const { nodeCount, graphType, numClusters, linkage, seed } = args;
    ...
    `;
    infoPanel.innerHTML = `
        <div style="font-weight: 600; margin-bottom: 8px; color: #1e293b;">Hierarchical Clustering (${linkage} linkage)</div>
    ...
    updateStatus(statusPanel, "Click 'Compute Clusters' to run hierarchical clustering");
    ...
    function updateLegend(): void {
        ...
        for (let i = 0; i < clusters.length; i++) {
            ...
            item.innerHTML = `
                ...
                <span style="color: #475569;">Cluster ${i + 1} (${clusters[i].length} nodes)</span>
            `;
    ...
    function updateClustersDisplay(): void {
        ...
        for (let clusterIndex = 0; clusterIndex < clusters.length; clusterIndex++) {
            ...
            for (const nodeId of clusters[clusterIndex]) {
    ...
    function apply(): void {
        ...
        for (let i = 0; i < clusters.length; i++) {
            for (const nodeId of clusters[i]) {
                nodeToCluster.set(nodeId, i);
            }
        }
        ...
        updateStatus(statusPanel, `Found ${clusters.length} clusters using ${linkage} linkage`);
    }
...
const meta: Meta<HierarchicalArgs> = {
    title: "Clustering",
    argTypes: {
        nodeCount: {
...
type Story = StoryObj<HierarchicalArgs>;

/**
 * Hierarchical clustering story with before/after visualization.
 *
 * This story uses the actual `hierarchicalClustering()` function from @graphty/algorithms.
 * Nodes are colored by their cluster membership.
 */
export const Hierarchical: Story = {
    render: (args) => createHierarchicalStory(args),
