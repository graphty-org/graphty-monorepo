/**
 * Label Propagation Community Detection Algorithm Stories
 *
 * Demonstrates label propagation community detection with a before/after visualization.
 * Shows how nodes are colored by their detected community membership.
 *
 * IMPORTANT: This story uses the actual labelPropagation implementation
 ...
 */
interface LabelPropagationArgs {
    nodeCount: number;
    graphType: GraphType;
...
/**
 * Create the Label Propagation visualization story.
 */
function createLabelPropagationStory(args: LabelPropagationArgs): HTMLElement {
    const { nodeCount, graphType, maxIterations, seed } = args;
    ...
    for (const [nodeId, label] of result.communities) {
    ...
    // Use local communities variable for the rest of the code
    const communities = communitiesArray;
    ...
    `;
    infoPanel.innerHTML = `
        <div style="font-weight: 600; margin-bottom: 8px; color: #1e293b;">Label Propagation</div>
    ...
    updateStatus(statusPanel, "Click 'Detect Communities' to run label propagation");
    ...
    function updateLegend(): void {
        ...
        for (let i = 0; i < communities.length; i++) {
            ...
            item.innerHTML = `
                ...
                <span style="color: #475569;">Community ${i + 1} (${communities[i].length} nodes)</span>
            `;
    ...
    function updateCommunitiesDisplay(): void {
        ...
        for (let commIndex = 0; commIndex < communities.length; commIndex++) {
            const color = COMMUNITY_COLORS[commIndex % COMMUNITY_COLORS.length];
            for (const nodeId of communities[commIndex]) {
    ...
    function apply(): void {
        ...
        // Create node to community index mapping
        const nodeToCommunity = new Map<string, number>();
        for (let i = 0; i < communities.length; i++) {
            for (const nodeId of communities[i]) {
        ...
        updateStatus(statusPanel, `Found ${communities.length} communities in ${result.iterations} iterations`);
    }
...
const meta: Meta<LabelPropagationArgs> = {
    title: "Community",
    argTypes: {
        nodeCount: {
...
type Story = StoryObj<LabelPropagationArgs>;

/**
 * Label Propagation community detection story with before/after visualization.
 *
 * This story uses the actual `labelPropagation()` function from @graphty/algorithms.
 * Nodes are colored by their detected community membership.
 */
export const LabelPropagation: Story = {
    render: (args) => createLabelPropagationStory(args),
