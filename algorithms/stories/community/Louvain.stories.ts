/**
 * Louvain Community Detection Algorithm Stories
 *
 * Demonstrates Louvain community detection with a before/after visualization.
 * Shows how nodes are colored by their detected community membership.
 *
 * IMPORTANT: This story uses the actual louvain implementation
...
/**
 * Create the Louvain visualization story.
 */
function createLouvainStory(args: LouvainArgs): HTMLElement {
    ...
    `;
    infoPanel.innerHTML = `
        <div style="font-weight: 600; margin-bottom: 8px; color: #1e293b;">Louvain Community Detection</div>
    ...
    updateStatus(statusPanel, "Click 'Detect Communities' to run Louvain algorithm");
    ...
    function apply(): void {
        ...
        for (let i = 0; i < result.communities.length; i++) {
            for (const nodeId of result.communities[i]) {
                nodeToCommunity.set(nodeId as number, i);
            }
        }
        ...
        // Apply colors to edges (color by source community)
        const edges = svg.querySelectorAll("[data-source]");
        edges.forEach((edge) => {
            const source = parseInt(edge.getAttribute("data-source") ?? "0");
            const target = parseInt(edge.getAttribute("data-target") ?? "0");
...
type Story = StoryObj<LouvainArgs>;

/**
 * Louvain community detection story with before/after visualization.
 *
 * This story uses the actual `louvain()` function from @graphty/algorithms.
 * Nodes are colored by their detected community membership.
 */
export const Louvain: Story = {
    render: (args) => createLouvainStory(args),
