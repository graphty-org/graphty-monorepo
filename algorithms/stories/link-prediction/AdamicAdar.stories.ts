/**
 * Adamic-Adar Link Prediction Algorithm Stories
 *
 * Demonstrates Adamic-Adar link prediction with a before/after visualization.
 * Shows predicted edges as dashed lines with their scores.
 *
 * IMPORTANT: This story uses the actual adamicAdarPrediction implementation
...
function createAdamicAdarStory(args: AdamicAdarArgs): HTMLElement {
    ...
    legend.innerHTML = `
        ...
        <span style="display: inline-flex; align-items: center; gap: 4px;">
            <span style="width: 20px; height: 2px; background: #8b5cf6; border-style: dashed;"></span>
    ...
    `;
    infoPanel.innerHTML = `
        <div style="font-weight: 600; margin-bottom: 8px; color: #1e293b;">Adamic-Adar Predictions</div>
        <div style="font-size: 11px; color: #64748b; margin-bottom: 8px;">
            Score = Σ 1/log(degree) for common neighbors
        </div>
    ...
    function updatePredictionsDisplay(): void {
        ...
        for (const pred of predictions) {
            ...
            badge.style.cssText = `
                ...
                font-weight: 600;
            `;
            badge.textContent = `${pred.source}↔${pred.target} (${pred.score.toFixed(2)})`;
    ...
    function apply(): void {
        ...
        if (edgeGroup) {
            for (const pred of predictions) {
                ...
                if (sourceNode && targetNode) {
                    ...
                    text.textContent = pred.score.toFixed(1);
        ...
        nodes.forEach((node) => {
            ...
            if (involvedNodes.has(nodeId)) {
                (node as SVGCircleElement).setAttribute("stroke", "#8b5cf6");
...
const meta: Meta<AdamicAdarArgs> = {
...
type Story = StoryObj<AdamicAdarArgs>;

/**
 * Adamic-Adar link prediction story with before/after visualization.
 *
 * This story uses the actual `adamicAdarPrediction()` function from @graphty/algorithms.
 * Predicted edges are shown as dashed purple lines with their scores.
 * The Adamic-Adar index gives more weight to common neighbors with fewer connections.
 */
export const AdamicAdar: Story = {
    render: (args) => createAdamicAdarStory(args),
