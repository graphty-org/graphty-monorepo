 ...
 *
 * ```tsx
 * import { FieldGlyph, PanelField, UiGlyph } from "@graphty/compact-mantine";
 *
 * <PanelField label="Edge width" glyph="width" kind="number" defaultValue={2} />
 * <FieldGlyph name="opacity" filled size={14} />
 * <UiGlyph name="settings" />
 * ```
 *
 * `FIELD_GLYPH_NAMES`, `FIELD_LETTERS` and `UI_GLYPH_NAMES` list the registers, and
 * `isFieldGlyphName` / `isFieldLetter` narrow an unknown string to them. Your own tools in the
 * app shell bring their own icons, drawn to sit in a 24px box.
 *
 * ## Accessibility
 *
 * Glyphs are `aria-hidden`. The control around a glyph carries the name: a `PanelField`'s
 * `label`, an icon button's `aria-label`.
 */
const meta: Meta<typeof FieldGlyph> = {
    title: "Foundations/Glyphs",
    component: FieldGlyph,
    subcomponents: { UiGlyph },
    parameters: {
        layout: "padded",
    },
};
...
/**
 * The shared marks, drawn everywhere except inside a field's slot.
 *
...
export const UiGlyphs: Story = {
    render: (): React.JSX.Element => (
        <Gallery caption={`${String(UI_GLYPH_NAMES.length)} marks, drawn everywhere except inside a field's slot.`}>
