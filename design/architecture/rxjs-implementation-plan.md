...
export class LayoutCoordinator {
    ...
    private setupAutoLayout(): void {
        ...
        this.dataManager.dataChanges$
            .pipe(
                filter((change) => change.type === "nodes-added" || change.type === "edges-added"),
...
export class CancellationTester {
    testSwitchMap(): void {
        ...
        trigger$.pipe(switchMap((n) => this.longOperation(n).pipe(finalize(() => completeCount++)))).subscribe();
    ...
    testExhaustMap(): void {
        ...
        trigger$.pipe(exhaustMap((n) => this.longOperation(n))).subscribe();
    ...
    private longOperation(n: number): Observable<void> {
        return timer(100).pipe(map(() => console.log(`Operation ${n} complete`)));
    }
}
```
...
export class ProgressTracker {
    ...
    trackOperation<T>(operationId: string, operation$: Observable<T>): Observable<T> {
...
export class RxGraphOperationManager {
    ...
    private processStyleCommand(cmd: Command): Observable<void> {
        return this.updateStyles(cmd).pipe(switchMap(() => this.updateMeshes()));
    }
    ...
    private processDataCommand(cmd: Command): Observable<void> {
        ...
        const dataLoad$ = this.updateData(cmd).pipe(
            ...
            switchMap(() => {
                ...
                return this.shouldAutoTriggerLayout() ? this.triggerLayout() : this.updateRender();
            }),
...
test("data loading completes before layout starts", async () => {
    ...
    manager.allOperations$.subscribe((op) => events.push({ type: op.type, timestamp: Date.now() }));
    ...
    assert.equal(
        dataComplete.timestamp < layoutStart.timestamp,
        true,
...
export class ObsolescenceManager {
    ...
    executeWithObsolescence<T>(id: string, type: string, operation$: Observable<T>): Observable<T> {
...
test("obsolescence cancels running operations", async () => {
    ...
    const layout$ = timer(100).pipe(tap(() => (layoutCompleted = true)));

    manager.executeWithObsolescence("layout-1", "layout:update", layout$).subscribe();

    await delay(50); // Layout still running

    // Data operation should cancel layout
    const data$ = of(true).pipe(tap(() => (dataCompleted = true)));

    await manager.executeWithObsolescence("data-1", "data:add", data$).toPromise();
...
export class MemoryLeakDetector {
    ...
    checkLeaks(owner: object): number {
        ...
        const activeCount = Array.from(subs).filter((sub) => !sub.closed).length;
