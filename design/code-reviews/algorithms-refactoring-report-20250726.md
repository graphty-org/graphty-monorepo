...
export function getCommonNeighbors(graph: Graph, source: NodeId, target: NodeId, directed?: boolean): Set<NodeId>;
...
export function initializeDistanceMatrix<T>(nodes: T[], defaultValue: number = Infinity): Map<T, Map<T, number>>;
...
export function calculateModularity(graph: Graph, communities: Map<NodeId, number>): number;
...
export function bfsDistancesOnly(graph: Graph, source: NodeId, cutoff?: number): Map<NodeId, number>;
