...
| The 1.x result projection | `node.algorithmResults`, `edge.algorithmResults`, `DataManager.graphResults`, `Algorithm.results`, and the three writers `Algorithm.addNodeResult` / `addEdgeResult` / `addGraphResult`, plus the internal projection that filled them from a finished result | REMOVED. A run publishes a `RunResult`; the 1.x entry point `run()` leaves it on the algorithm, readable as `algorithm.result` | The projection existed for exactly one consumer: the hand-written suggested layers, whose selectors and calculated-style inputs named `algorithmResults.<namespace>.<type>.<key>`. With those gone it had no reader | Read `algorithm.result`, or start the work as a run and read `run.result`. A plugin algorithm returns an `AlgorithmOutput` from `compute()` instead of calling the three writers | NO |
...
| `StyleManager` is deleted | `StyleManager` (`src/managers/StyleManager.ts`, 286 lines, 17 public methods), exported from the barrel among the managers, reachable as `Graph.getStyleManager()` and `Graphty.getStyleManager()` | REMOVED. The layer verbs that still speak the 1.x index vocabulary are on `Graph`: `addStyleLayer`, `insertStyleLayer`, `removeStyleLayersByMetadata`, `removeStyleLayerByIndex`, `updateStyleLayerByIndex`, `reorderStyleLayers`, `getLayers`. Everything else is `session.styles` | Sixteen of the seventeen methods forwarded to `Styles` unchanged; the seventeenth cached a resolved style id against `JSON.stringify(data)`, which is a string allocation per element per resolution. A wrapper that adds a cache nobody can invalidate correctly is not a layer of the design | `element.session.styles` for new code; `graph.addStyleLayer(...)` where a 1.x layer object is still in hand | PARTIAL |
...
Of the ninety-three surface changes, nineteen are mechanically fixable, twenty-six are
