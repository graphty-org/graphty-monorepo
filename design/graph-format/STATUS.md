# Status: @graphty/graph-format and @graphty/graph-io

Implementation record for the two packages (`design/graph-format/graph-format-design.md` is the
normative design; section numbers below refer to it).

Both packages LANDED in graphty-monorepo on 2026-09-16 as workspace members `graph-format/` and
`graph-io/` (design section 14.6 phase F1 plus the IO1 package landing). This file was written while
they were staged in `webgpu-graph-algorithms/packages/`, and was copied here with them so the record
travels with the code; the staging copy is historical. Paths of the form `packages/graph-format` in
...
## F1 landing in graphty-monorepo (2026-09-16)
...
the landing, so the pre-push hook is green.

`pnpm exec prettier --check graph-format graph-io` reports seven files the monorepo's prettier 3.7.4
formats differently from the staging workspace's 3.9.6 (the checklist predicted six; the seventh is
`graph-io/test/audit/conformance-exports.test.ts`, added in audit round 1), plus the parse errors on
the two deliberately malformed JSON corpus fixtures. Prettier is not enforced by this repo's CI or
hooks and the files were left as they are.

### Defect found and fixed: the Pajek vertex section was quadratic

The landing benchmark run measured the Pajek importer at 3.34 us/edge against 0.79-2.51 for every
other importer, and a doubling series that barely moved (2563 / 2599 / 2619 ms for 25k / 50k / 100k
arcs against a vertex count pinned at 100k) -- about 2.4 s for the vertex section alone, 25 us per
vertex.

Cause: under the default options (`nodeIdFrom: "id"` plus `restoreMangledIds: true`) `restoresIds()`
is true, so `verticesHeader` allocates `idOfPos` for every file whether or not any line carries a
`graphty_originalId`, `createNodes()` early-returns, and every vertex line takes the restore-mode
branch of `vertexLine`. That branch's gap-fill loop restarted at position zero on each line. With
vertex numbers in ASCENDING order -- the order Pajek itself writes, and the order the fixture uses --
each line scans the positions already filled, for n(n-1)/2 iterations in total. Descending or
shuffled line order fills everything on the first line and runs linearly, which is why no corpus test
saw it.

Fix: a `filledBelow` high-water mark so the gap-fill resumes where it stopped (one private field and
three lines in `PajekParser.vertexLine`, reset in `verticesHeader`). No public surface, option, issue
code, id, node order or report field changes. Equivalence was checked over 3200 randomised cases
(shuffled, gapped and duplicated vertex lines, `graphty_originalId` parameters, coordinates, shape
...
Regression test: `test/formats/pajek/vertex-scaling.test.ts`, deliberately UNGATED (the audit suite's
`streaming-quadratic.test.ts` is gated on `IO_BENCH=1` and compares input shapes at one fixed size,
never doubling the vertex count, which is why it could not catch this). It quadruples the vertex
count and requires under 8x the time; linear is about 4x and the defect was about 16x.

### Pre-1.0 gaps, revisited
...
1. GEXF whole-document parsing -- NOT A GAP, the note was stale. GEXF runs one SAX-style pass:
   ...
   50k -> 400k, peak +26.7 MiB against a 14.8 MiB input. GEXF is faster than GraphML, not the 3x
   slower that would have forced a rewrite. It is GraphML, not GEXF, that still builds bounded
   per-`<data>` yFiles trees.
2. Hot-loop allocations -- recorded, not removed, and the recorded reason for the headline site holds.
   The GEXF node frame's `IssueLocation` is pushed into `parentRefs` / `parentsRefs` and read by
   `resolveParents()` at `</graph>` or at the start of `<edges>`, long after the frame is popped at
   the node's end tag, and `<nodes>` nests inside `<node>` so several frames are live at once: a
   single reused location is impossible, and a depth-indexed pool would need clone-on-escape at three
   sites plus an audit of seven `frame.where` reads. Not local, and the pattern that made the edge
   frame reusable does not transfer (edges cannot nest and every edge-location consumer copies the
   fields synchronously). The DOT per-edge defaults copy and the Pajek per-line extras record are
   local and could be elided; they were left alone because all three sites measure 2-3 us per edge,
   in line with every other importer, and the landing already changed one Pajek hot path.
   Two corrections to the wording of the gap at the top of this file: the GEXF node path builds no
   string (`idText ?? attrs.get("label") ?? null` is a reference the tokenizer's attribute map already
   holds), so "eager element strings" belongs to DOT and Neo4j; and for Pajek it is the token array,
   not the extras record, that resists removal, because `tokenize()` lives in `syntax.ts` and encodes
   the unterminated-quote failure in its `string[] | null` return.
3. Compaction after removals -- recorded as a design correction, not optimised. Measured with
   `freezeWithReport({ profile: true })` at 100k nodes / 1M directed edges, median of 5: the
   `compact` phase is 31.78 ms against the 10-15 ms of design 15.4 and 6.7, in a freeze whose total
   goes 21.04 ms -> 52.50 ms when 10 percent of the edges are tombstoned first. Both design tables
   now carry the measured number and the full phase profile is design section 15.6. The design's
   estimate was wrong, not the code: the dominant term is the incidence-list rebuild, four random
   accesses per surviving edge, which is cache-miss bound like the counting sort rather than the
   memcpy class the estimate assumed.

### Landing deviations

- The two package directories were COPIED from the staging workspace, not moved: another session held
  that repository open at landing time. The staging copies are untouched and are now historical.
- This file and CONFORMANCE.md were copied to `design/graph-format/` beside the design document, for
  the same reason and so the record travels with the code.
- `graph-format/test/build-output.test.ts` guarded its bundle assertions on `dist/graph-format.js`
  existing. Since the rehearsal added the `graph-format.ts` root entry, a tsc-only build -- which is
  what `tools/prepush.sh` runs -- also writes that file, as a re-export stub with a sourceMappingURL
  comment, so the guard passed and the test then failed against the stub. The guard now recognises the
