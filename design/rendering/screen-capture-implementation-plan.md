...
- `src/screenshot/transparency.ts`: Transparent background handling
    ...
    export async function enableTransparentBackground(scene: Scene, skybox: Mesh | null): Promise<BackgroundState> {
    ...
    export function restoreBackground(scene: Scene, skybox: Mesh | null, state: BackgroundState): void {
...
- `src/screenshot/presets.ts`: Screenshot preset definitions
    ...
    export function resolvePreset(preset: string, overrides?: Partial<ScreenshotOptions>): ScreenshotOptions {
        const presetConfig = SCREENSHOT_PRESETS[preset];
        if (!presetConfig) {
            throw new ScreenshotError(`Unknown screenshot preset: ${preset}`, ScreenshotErrorCode.PRESET_NOT_FOUND);
        }
...
- `src/camera/CameraStateManager.ts`: Unified camera state management
    ...
    export class CameraStateManager {
        ...
        private async animateArcRotateCamera(
            ...
            if (state.position) {
                const posAnim = new Animation("camera_position", "position", fps, Animation.ANIMATIONTYPE_VECTOR3);
                posAnim.setKeys([
            ...
            if (state.target) {
                const targetAnim = new Animation("camera_target", "target", fps, Animation.ANIMATIONTYPE_VECTOR3);
                targetAnim.setKeys([
...
- `test/browser/camera/camera-presets-3d.test.ts`: Built-in 3D presets
    ...
    test("3D-only presets throw error when used with 2D camera", () => {
        ...
        assert.throws(() => graph.resolveCameraPreset("sideView"), {
            name: "ScreenshotError",
            code: "CAMERA_PRESET_NOT_AVAILABLE_IN_2D",
        });

        assert.throws(() => graph.resolveCameraPreset("frontView"), {
            name: "ScreenshotError",
            code: "CAMERA_PRESET_NOT_AVAILABLE_IN_2D",
        });

        assert.throws(() => graph.resolveCameraPreset("isometric"), {
            name: "ScreenshotError",
            code: "CAMERA_PRESET_NOT_AVAILABLE_IN_2D",
        });
    });
...
- `test/browser/camera/camera-presets-user-defined.test.ts`: User-defined presets
    ...
    test("cannot overwrite built-in presets", () => {
        assert.throws(() => graph.saveCameraPreset("fitToGraph"), {
            name: "ScreenshotError",
...
- `test/browser/screenshot/screenshot-error-codes.test.ts`: Error code coverage
    ...
    test("throws ENGINE_NOT_CONFIGURED when preserveDrawingBuffer is false", async () => {
        ...
        await assert.rejects(() => graph.captureScreenshot(), {
            name: "ScreenshotError",
    ...
    test("throws DIMENSION_TOO_LARGE when exceeding browser limits", async () => {
        await assert.rejects(() => graph.captureScreenshot({ width: 20000, height: 20000 }), {
            name: "ScreenshotError",
            code: "DIMENSION_TOO_LARGE",
        });
    });
    ...
    test("throws WEBP_NOT_SUPPORTED when browser lacks WebP", async () => {
        ...
        vi.spyOn(document.createElement("canvas"), "toBlob").mockImplementation((callback, type) => {
            if (type === "image/webp") {
                throw new Error("WebP not supported");
            }
        });

        await assert.rejects(() => graph.captureScreenshot({ format: "webp" }), {
            name: "ScreenshotError",
    ...
    test(
        "throws LAYOUT_SETTLE_TIMEOUT when layout never settles",
        async () => {
            const layoutEngine = new MockLayoutEngine();
            layoutEngine.setSettled(false);
            graph.setLayoutEngine(layoutEngine);
    ...
    test("throws CAMERA_PRESET_NOT_FOUND for unknown preset", async () => {
        await assert.rejects(() => graph.captureScreenshot({ camera: { preset: "nonexistent" } }), {
            name: "ScreenshotError",
    ...
    test("throws CAMERA_PRESET_NOT_AVAILABLE_IN_2D for 3D-only presets", async () => {
        ...
        await assert.rejects(() => graph.captureScreenshot({ camera: { preset: "isometric" } }), {
            name: "ScreenshotError",
            code: "CAMERA_PRESET_NOT_AVAILABLE_IN_2D",
        });
    });
...
- `test/browser/video/video-capture-stationary.test.ts`: Basic video capture with mocks
    ...
    test("auto-detects best supported codec", () => {
        ...
        assert.ok(codec.includes("vp9") || codec.includes("vp8"));
    });
...
- `src/video/MediaRecorderCapture.ts`: MediaRecorder wrapper with Safari support
    ...
    export class MediaRecorderCapture {
        ...
        async captureRealtime(canvas: HTMLCanvasElement, options: AnimationOptions): Promise<AnimationResult> {
...
- `src/video/estimation.ts`: Pre-flight estimation
    ...
    ```typescript
    export async function estimateAnimationCapture(options: AnimationOptions): Promise<CaptureEstimate> {
...
_[Similar testing pattern: mock dependencies, test our logic, one E2E smoke test]_
