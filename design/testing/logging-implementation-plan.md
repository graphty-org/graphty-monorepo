...
- `src/logging/sinks/ConsoleSink.ts`: Browser console output
    ```typescript
    export function createConsoleSink(options?: { formatter?: (record: LogRecord) => string; colors?: boolean }): Sink;
    ```
...
- Update `src/logging/index.ts`: Complete public API
    ...
    ```typescript
    export { GraphtyLogger, LogLevel, configureLogging, getLoggingConfig } from "./GraphtyLogger";

    export type { LogRecord, Sink, LoggerConfig, Logger } from "./types";

    export { createConsoleSink, createRemoteSink } from "./sinks";
