...
Execution note (2026-09-17, runs 35268079740 / 35288880508 on PR #11): the first run died in `Build` because `nx affected` had never run on a `pull_request` event (`defaultBase: master` names a ref a PR checkout does not have) -- fixed on the branch as `ci: name the PR base for nx affected` (`--base=origin/${{ github.base_ref }} --head=HEAD`). The second run: `Build` and every Chromatic job green; `Test (webgpu-graph-algorithms-node)` GREEN on lavapipe (`lvp_icd.json` found by the `find` step, Mesa 25.2.8, coverage thresholds met); `Test (webgpu-graph-algorithms-browser)` 37/39 with ONE failure, `test/browser/uniform-layout.test.ts` on Chromium 143 (unlisted `uniform_buffer_standard_layout`, struct accepted under `--enable-unsafe-webgpu` -- the P2-T1 measurement in the test's own header, now the CI runtime); the same failure on both Hosts, plus every WebKit GPU test failing with `E_NO_ADAPTER` (32 failed / 6 passed / 1 skipped: Playwright 1.57's WebKit 26.0 has no `navigator.gpu`; 26.6 on 1.63 does). Fixed as three follow-up commits on the branch: `test(webgpu-graph-algorithms)` -- the browser test expects acceptance on Chromium < 145 (144 unmeasured) whether or not the feature is listed, requires it listed on 145+, and keeps `accepted === listed` on every other engine (WebKit 26.6 rejects: the measured rejected leg); verified 38/39 on Chromium 143 (worktree) and on Chromium 153 (a Playwright 1.63 scratch copy); `ci` -- hosts.yml probes WebKit for `navigator.gpu` (`scripts/probe-webkit-webgpu.mjs`) and skips the WebKit smoke when absent, so the lane reports the toolchain fact instead of a vacuous red; `docs` -- this note and D-11. The root Playwright bump was measured and rejected (D-11). `Test (graph-format)` and `Test (compact-mantine)` stay red until the owner fixes the version constants on master.
 ...
 * @graphty/webgpu-graph-algorithms is imported only by graphty-element, from its optional
 * @graphty/graphty-element/webgpu entry point (design 9.1).
 */
...
- [ ] **Step 1:** Append to `design/graph-format/graph-format-design.md` after its last section-17 entry a `### 17.8 Amendments from the WebGPU design (W1, <date>)` table with one row per departure, each citing the WebGPU design line: DEPARTURE-1 (node-first testing with a browser smoke: amends 14.5 "browser-only vitest project", 14.6's W1 gate "browser tests green" and 16.7), DEPARTURE-2 (the hot-prefix arena upload: amends 10.3), DEPARTURE-4 (the per-snapshot residency record: amends 14.5), DEPARTURE-5 (injection spelled `accelerated(acc).x(s, options)`: amends 14.5), DEPARTURE-6 (1e-4 for betweenness and one-iteration force parity: amends 16.2). The DEPARTURE texts are `design/webgpu/webgpu-acceleration-plan.md:225-237`.
- [ ] **Step 2:** Append to the WebGPU design's Review log: "W1b landed <the commit date, YYYY-MM-DD>: the W1 amendments are graph-format-design.md 17.8", together with the layout half (mirrors -> imports, layout's ForceAtlas2Simulation as the second FA2 reference, the seedPositions cross-test).
