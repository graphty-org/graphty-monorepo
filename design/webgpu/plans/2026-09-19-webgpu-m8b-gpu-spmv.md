...
**PLAN DECISION PD-7 (the ping-pong is two buffers and two cached bind groups; recorded as DEP-M8B-G).** Nothing forbids one buffer of two 256-aligned halves: `Kernel.bind`'s aliasing check is per kernel (`src/kernel/kernel.ts:170-205`), `pr-scale` binds both halves `storage-ro` (a pair the check skips) and `spmv-pull` binds one half alone as `rankOut` (no pair to check), and WebGPU's usage scope is per dispatch. The one-buffer form is bindable and is rejected because it gains next to nothing over two leases: the same two cached bind groups, the same one `dispose()`, pool bytes that are the same for `512 < n <= 8,388,608` and at most a few classes apart elsewhere (`BufferPool.sizeClass`, `src/memory/buffer-pool.ts:73-89`; the bands are in the record), half the nodes under the per-buffer `E_TOO_LARGE` check (`src/memory/buffer-pool.ts:99-108`), plus a computed midpoint offset in a driver whose every other scratch binding is a whole buffer at offset 0 (the residency and the force driver do bind sub-ranges, `src/memory/residency.ts:588,791`, `src/layouts/force-simulation.ts:1697-1698`; the algorithm drivers do not). So `rankIn` and `rankOut` are TWO buffers, A and B, and the ping-pong is two calls to `kernel.bind()` with the two resource sets; `bind()` caches by `bufferId:offset:size` joined by `|` (`src/kernel/kernel.ts:206-209`), so alternating them costs one Map lookup per iteration and allocates nothing after the first two. Every binding offset is 0 and every scratch buffer comes from the `Lease`, so the 256-byte rule (`src/kernel/kernel.ts:157-166`, `STORAGE_ALIGN`) is satisfied trivially -- with ONE exception to watch: the `partials` binding covers the whole partials buffer from offset 0, and the header lives at element 0 INSIDE it, not at a separate binding, precisely so no sub-256 offset is ever needed.
