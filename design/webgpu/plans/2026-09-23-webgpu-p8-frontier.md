...
Every row was counted on master with the command in its evidence column. Master has moved since (release and element commits that touch none of the counted paths, re-checked by the 2026-09-24 review), so every fact below still holds, but branch `feat/gpu-p8` is no longer master's tip and needs a merge from master before its pull request.
...
result batch: keys = scope.scratch(4n); batch.copy(depth -> keys); vals = fill mode 1 (iota); radixSort.record(pass, keys, vals, n, 32, scratch) -> sorted;
              pred = fill INVALID_INDEX; sssp-pred MODE 1, predKind 1 over plan = planGridStride(n, WG, caps) with stride = plan.stride and source; four readback requests: depth (4n), pred (4n), sorted.vals (4n), counters (96)
...
- [ ] **Step 4: Proof.** Against `bellmanFordOracle` (f32) and the f64 Dijkstra where weights are non-negative: the P8-T9 fixture list plus graphs with negative but acyclic weights (directed), a planted negative cycle (the gate's item), a negative cycle unreachable from the source (the flag must be FALSE -- this is the case a naive implementation gets wrong), a zero-weight cycle (also false), an undirected graph with a negative edge (which IS a negative cycle of length two, and the flag must say so) and `cutoff`, with P8-T9 Step 6's normalisation cases repeated here so `bellmanFord` cannot drift from `sssp` on the same option: `cutoff: NaN` is `E_INVALID_ARGUMENT { argument: "cutoff" }` before any device work, `cutoff: -1` is the source alone, `cutoff: Infinity` equals no cutoff. Two override cases: `weights` with non-unit values (integer weights 1..10, seeded) on an UNWEIGHTED snapshot (`pathEdges(500)` with no weight column) gives `dist` bitwise equal to the f32 oracle run with that vector and NOT equal to the BFS depths as f32 (the option is not dropped, PD-22); and on an UNDIRECTED snapshot an override whose two arcs of one edge differ (the symmetric vector with `weights[edgeToArc[arcToEdge[a]]] + 1` written into ONE reverse arc) is `E_UNSUPPORTED` with `details.feature === "bellmanFord.asymmetricUndirectedWeights"`, thrown before any device work (`counter.mapAsyncCalls` unchanged after `verifyDevice` and a reset), while the same override made symmetric again runs and equals `sssp`'s answer bitwise. `dist` bitwise against the f32 oracle, within the derived tolerance of the f64 one, and cross-checked bitwise against `sssp` on the same graph when all weights are non-negative; `expectPredArcAttains`; run-twice bitwise on `dist`, `pred` and the flags; `retryExhaustedRounds === 0` on every fixture, so the bound is known to be adequate rather than assumed. Every readback names its buffer.
...
- [ ] **Step 3: The bit-parallel sweep.** Sources are taken 32 per batch (`ceil(n / 32)` batches). The batch's state is one `bits` buffer of four regions of `bitsBase = roundUp(n, 64)` words each -- `visited` at 0, `frontier` at `bitsBase`, `next` at `2 x bitsBase`, `flags` at `3 x bitsBase` (each region's byte offset is 256-aligned, so `fill` and `compact` can bind one alone) -- with bit `s` of word `v` meaning "source `s` has reached / is at / is next at `v`"; a `frontierList` of `n` words with its count in `counters[0]`; and `perSource`, 128 words: `newCount[32]` @0, `reached[32]` @32, `sumLo[32]` @64, `sumHi[32]` @96. The sweep is `advance-expand`'s block-mapped expansion (P8-T5 Step 2: the strip copied textually; the scan INLINED as `bfs-contract`'s Hillis-Steele, not the `wg_scan_u32` call, because this kernel is not a twin kernel and declares `needs: []`, P8-T1 Step 5) with the claim inline: for the arc `(u, x)`, `mask = frontier[u] & ~visited[x]`; if non-zero, `old = atomicOr(&visited[x], mask)`, `fresh = mask & ~old`; if non-zero, `atomicOr(&next[x], fresh)`, `flags[x] = 1`, and one `atomicAdd` on a WORKGROUP-memory `local[s]` per set bit of `fresh` (`firstTrailingBit` and `b & (b - 1u)`), flushed after the strip loop by lanes 0..31 into `perSource.newCount` -- one global atomic per source per workgroup, never one per arc. The loop's bound is the `workgroupUniformLoad` aggregate, so the flush's barrier is uniform. Bindings: the four graph slots, `(1,0) frontierList ro array<u32>`, `(1,1) counters rw array<atomic<u32>>`, `(1,2) bits rw array<atomic<u32>>`, `(1,3) perSource rw array<atomic<u32>>`, `(2,0) P` (`bitsBase`, `arcBase 0`, `arcEnd arcCount`; never windowed); storage count 8. The frontier and next regions swap by parity through `P.mode`, so no copy runs between levels.
...
Fifteen kernels (section 0.4 PD-4 and the table of P8-T4 Step 2: three primitives' bodies, one selector, one expansion, three BFS claims, two BFS helpers, one predecessor pass, one relax, one Bellman-Ford relax, two closeness). Nine appends to `src/kernels.ts` (P8-T3 .. P8-T11). Seven indirect slots per level (`FRONTIER_CANDIDATES`). Twenty-four words in the counters block, 96 bytes; twenty fields in the params block, 80 bytes. Fifty-two sabotage rows (the forty-nine of the 2026-09-23 count plus the three the done-boundary rule and the striding predecessor pass added) against a floor of forty-five. Six constants and one WGSL helper pair. Five decision records. Eighteen commits. Twelve gate items. 22.5 engineer-days.
