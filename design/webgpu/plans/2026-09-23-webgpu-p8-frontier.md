...
Both forms carry their own barriers, so a caller reaches them in uniform control flow only (the guarded-loads-into-locals rule of P8-T5 Step 2). The five other P8 bodies that scan (`dedupe-filter`, `bfs-contract`, `bfs-fused`, `bfs-bottom-up`, `closeness-sweep`) keep their INLINED Hillis-Steele and `needs: []`: none is a twin kernel, their sabotage rows pin the inlined text, and the scan decision record's argument (two bodies that cannot differ prove nothing) applies to each of them. Only `advance-expand` calls the helper.
