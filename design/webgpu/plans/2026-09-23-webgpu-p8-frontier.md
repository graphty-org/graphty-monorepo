...
Every row was counted on master with the command in its evidence column. Master has moved since: `origin/master` is 26 commits past `6dbabfc0` (PRs #339-#352: graph-io fixes, element fixes, one release and a `release.yml` change), none of which touches `webgpu-graph-algorithms/`, `algorithms/src`, `design/`, `hosts.yml` or `gpu.yml`, and which touch `graphty-element` only in `MaxFlowAlgorithm.ts`, `MinCutAlgorithm.ts` and `utils/graphUtils.ts` (re-checked by the 2026-09-24 review), so every fact below still holds, but branch `feat/gpu-p8` is no longer master's tip and needs a merge from `origin/master` before its pull request. "Master" throughout means `origin/master`: the worktree's local `master` ref is stale at `f449e107`, BEHIND `6dbabfc0`, so `git log master` in the worktree shows an older master than this section describes.
...
fn frontier_finalize(@builtin(local_invocation_id) lid: vec3<u32>) {
    ...
    if (P.role == 0u) {                                                // the level boundary
        ...
        if (P.firstOfSubmit == 0u) {                                   // the submit's rebuild already excluded these (P8-T8, PD-18)
...
for each submit (while not done):
    ...
    for level in 0 .. levelsPerSubmit - 1:
        planner.recordFinalize(pass, 0, level, { ...fields, firstOfSubmit: level === 0 ? 1 : 0 })   // the level boundary: rotates the counts, chooses, writes this level's slots
...
| `frontier-finalize` | `planIndirect` and P4's `indirect-finalize` | 2,000 counts to `2^32 - 1`; 17M; both roles; the seed and rotation; the workgroup-per-entry slots; `maxDepth` | bitwise on the args | 4 + 2 (the selector rows of P8-T7 / P8-T8) | none |
