...
| PD-25 | `closenessCentrality` reads `weighted` (default: the snapshot's `flags.weighted`), and `maxIterations` / `tolerance` are accepted for the seam's type and documented as inapplicable to an exact traversal -- accepted as no-ops, never refused, so a later CPU port that ignores the same keys behaves identically through the dispatcher; `iterations` reports the source batches run, `converged` is always true | P8-T11, P8-T16 |
...
fn frontier_finalize(@builtin(local_invocation_id) lid: vec3<u32>) {
    ...
    if (P.role == 0u) {                                                // the level boundary
        ...
        if (atomicLoad(&counters[9]) > P.edgeCapacity) {               // PD-23: the fused retry
            ...
            atomicStore(&counters[18], atomicLoad(&counters[18]) + 1u);   // twoPhaseLevels counts the CHOICE, even for zero edges (P8-T7 Step 4's invariant)
        }
    }
}
`;
...
- [ ] **Step 2: PD-13 and PD-25 -- one driver, one output, the seam's options.** `score[s] = reached_s / sumDist_s`, scaled by `reached_s / (n - 1)` (the Wasserman-Faust rule for a disconnected graph), `0` when nothing is reached, computed in f64 on the host from exact integer sums and stored as `F32` with `precision: "f32"` -- the formula the legacy `closenessCentrality` of `@graphty/algorithms` produces by default, which is what graphty-element's closeness panel shows today, so a future `indexed.closenessCentrality` port has one number to match. `weighted` defaults to `s.flags.weighted`; when true AND the snapshot's weights are not all one, the run is one `sssp` per source with the per-source sums reduced on the host (design 8.4's own answer, slow and correct; `nonNegativeWeights === false` is `E_UNSUPPORTED`); `weighted: true` over `flags.allWeightsOne` (or no weight column) takes the bit-parallel sweep, one host branch, because every one of those `sssp` calls would route to a BFS anyway and the sweep gives the identical answer at roughly `1 / n` the cost. `maxIterations` and `tolerance` are accepted for the seam's type and have no effect: an exact traversal has neither, and the JSDoc says so. That is a recorded choice (PD-25; P8-T16's seam record): no-ops, never `E_UNSUPPORTED`, because the seam's dispatcher forwards the option bag blindly and a CPU `closenessCentrality` port that later ignores the same keys must not behave differently from the GPU member through the same call. `iterations` reports the source batches run, `converged` is always `true`. `dest` is `scores`.
