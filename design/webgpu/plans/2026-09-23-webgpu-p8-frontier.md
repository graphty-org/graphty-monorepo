...
export class Frontier {
    ...
    reset(queue: GPUQueue, source: number, seed: Partial<Record<keyof typeof W, number>>): void;   // queue.writeBuffer of the 96 B block (ZERO except the caller's seed words) and of vertices[0][0] = source, ordered before the submit
}
...
```ts
    "frontier-finalize": Object.freeze([
        ...
        { name: "fused-slot-per-invocation", find: "write_slot_groups(2u, next, next);", replace: "write_slot(2u, next);", minFactor: 10, test: "test/algorithms/bfs.test.ts" },
...
Expected: PASS on both; the 17M case green on lavapipe; the third and fourth sabotage rows are deferred to P8-T6's and P8-T7's runs and their `find` strings are asserted present by `test/sabotage/coverage.test.ts`.
