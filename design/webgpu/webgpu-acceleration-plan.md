...
> 7, 8, 9, 10, 12, 13, 14 and the Review log -- describe decisions that a later decision record
> reversed, and two of them leave an open question for the owner. The body below is deliberately
> NOT edited; `design/decisions/README.md` explains why a record names the lines it overrules
> instead. Which passages, what is true instead, which record decided it, and what is still open:
> `design/webgpu/superseded-parts-of-the-design.md`. This note moved the body down by eight lines,
> so a line number written before 2026-09-23 names text that now sits eight lines lower.
