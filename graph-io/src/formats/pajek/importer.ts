...
class PajekParser {
    ...
    /**
     * Every position below this one already has an id in `idOfPos`: the high-water mark of the
     * gap-fill in `vertexLine`. Without it the gap-fill restarts at zero on every vertex line, which
     * is quadratic in the vertex count for a file whose lines are in ascending order -- which is the
     * order Pajek itself writes.
     */
    private filledBelow = 0;
    ...
    private vertexLine(text: string, line: number): void {
        ...
        if (this.idOfPos !== null && !repeated && this.options.nodeIdFrom === "label") {
            ...
            for (let p = this.filledBelow; p < pos; p++) {
                if (this.idOfPos[p] === undefined) {
                    this.numberedNode(this.idOfPos, p);
                }
            }
            // after the loop on purpose: a throw part-way leaves the mark at its smaller, still
            // correct value and the next line re-scans the range
            if (pos > this.filledBelow) {
                this.filledBelow = pos;
            }
