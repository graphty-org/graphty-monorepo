 ...
 * input must roughly double the work; a super-linear ratio is a defect, and a chunk-size ratio
 * far above one means some per-token work scans the rest of the buffer.
 *
 * The work is counted, not timed: the characters the importer examines through the string
 * primitives it scans with (test/helpers/work-meter.ts). A count is the same on every run, so
 * the bounds hold on a loaded host; the pinned defects missed them by an order of magnitude or
 * more. Gated on IO_BENCH=1 for the size of the generated fixtures.
 */
...
/** The characters an import examines, and the edges it pushed. */
async function importWork(importer: GraphImporter, input: ImportInput): Promise<{ work: number; edges: number }> {
    const sink = new GraphBuilder({ directed: true, weightDtype: "f64" });
    const work = await charactersExamined(async () => {
        await importer.import(input, sink);
    });
    return { work, edges: sink.edgeCount };
}
