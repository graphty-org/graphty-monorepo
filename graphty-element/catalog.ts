 ...
 * ```
 *
 * Every algorithm, layout, file format, palette and scale the element offers is described here
 * as data: a plain name, a technical name, what it computes, what it costs and what it can be
 * configured with. An options form, a metrics panel, a file picker and a cost gate can all be
 * built from these tables without importing a class, a renderer or a 3D engine -- which is the
 * point, because the alternative is an application that hard-codes its own list and falls
 * silently out of step the first time the element gains or loses a capability.
 *
