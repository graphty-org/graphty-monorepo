...
12. **Using with Algorithms**:
    ...
    styleManager.addLayer({
        ...
        styles: {
            node: {
                color: (node) =>
                    StyleHelpers.color.sequential.viridis(node.algorithmResults["graphty:degree"] / maxDegree),
                size: (node) => StyleHelpers.size.log(node.algorithmResults["graphty:degree"], { min: 0.5, max: 2.5 }),
            },
        },
    });
    ```
