...
export class DataManager implements Manager {
    ...
    addNodes(nodes: Record<string | number, unknown>[], idPath?: string): void {
        ...
        for (const node of nodes) {
            const nodeId = this.nodeIdOf(node, idPath);
        ...
        try {
            ...
            try {
                ...
                if (replace) {
                    ...
                    // Settled BEFORE the graph goes, because this is the one check left that can
                    // refuse the whole file (E_EDGE_ENDPOINTS_UNRESOLVED): refusing it after the
