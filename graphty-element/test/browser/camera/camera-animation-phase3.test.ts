...
test("rapid successive camera animations - last one wins", async () => {
    ...
    assert.ok(Math.abs(finalState.position.x - 50) < 10, `Position X is ${finalState.position.x}, expected ~50`);
    assert.ok(Math.abs(finalState.position.y - 50) < 10, `Position Y is ${finalState.position.y}, expected ~50`);
    assert.ok(Math.abs(finalState.position.z - 50) < 10, `Position Z is ${finalState.position.z}, expected ~50`);
});
...
test("operation queue allows sequential animations", async () => {
    ...
    assert.ok(Math.abs(finalState.position.y - 50) < 10, `Final Y is ${finalState.position.y}, expected ~50`);
});
...
test("multiple rapid interruptions complete gracefully", async () => {
    ...
    assert.ok(Math.abs(state.position.x - 100) < 10, `Final position X is ${state.position.x}, expected ~100`);
});
