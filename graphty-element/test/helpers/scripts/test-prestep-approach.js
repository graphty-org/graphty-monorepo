...
async function preStepLayout(page, steps = 500) {
  return await page.evaluate(async (stepCount) => {
    ...
    if (!graphty) return { error: "No graphty-element found" };
    ...
    if (!graphty.graph) return { error: "No graph property after waiting" };
    
    const layout = graphty.graph.layoutManager?.layoutEngine;
    if (!layout) return { error: "No layout engine found after waiting", hasLayoutManager: !!graphty.graph.layoutManager };
