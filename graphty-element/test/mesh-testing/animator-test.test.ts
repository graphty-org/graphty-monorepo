...
describe("RichTextAnimator Tests", () => {
    ...
    describe("Pulse Animation", () => {
        test("animates scale with sine wave", () => {
            ...
            (animator as unknown as { updateAnimation: (mesh: unknown, material: unknown) => void }).updateAnimation(
                mesh,
                material,
            );
    ...
    describe("Shake Animation", () => {
        test("animates X and Y position with shake effect", () => {
            ...
            for (let i = 0; i < 100; i++) {
                ...
                (
                    animator as unknown as { updateAnimation: (mesh: unknown, material: unknown) => void }
                ).updateAnimation(mesh, material);
            ...
            testTimes.forEach((time, index) => {
                ...
                (
                    animator as unknown as {
                        updateAnimation: (mesh: unknown, material: unknown, callback?: unknown) => void;
                    }
                ).updateAnimation(mesh, material, progressCallback);
