...
describe("Accessibility - Colorblind Simulation", () => {
    ...
    describe("Categorical palettes accessibility", () => {
        it("Okabe-Ito palette is safe for protanopia", () => {
            ...
            assert.isTrue(safety.protanopia, "Okabe-Ito should be safe for protanopia (research-validated)");
        });

        it("Okabe-Ito palette is safe for deuteranopia", () => {
            const colors = Array.from({ length: 8 }, (_, i) => categorical.okabeIto(i));
            const safety = accessibility.isPaletteSafe(colors);
            assert.isTrue(safety.deuteranopia, "Okabe-Ito should be safe for deuteranopia (research-validated)");
        });

        it("Okabe-Ito palette is safe for tritanopia", () => {
            const colors = Array.from({ length: 8 }, (_, i) => categorical.okabeIto(i));
            const safety = accessibility.isPaletteSafe(colors);
            assert.isTrue(safety.tritanopia, "Okabe-Ito should be safe for tritanopia (research-validated)");
        });
        ...
        it("Paul Tol Vibrant palette is colorblind-safe", () => {
            ...
            const safety = accessibility.isPaletteSafe(colors);
            assert.isTrue(safety.protanopia, "Tol Vibrant should be safe for protanopia");
            assert.isTrue(safety.deuteranopia, "Tol Vibrant should be safe for deuteranopia");
        });

        it("Paul Tol Muted palette is colorblind-safe", () => {
            const colors = Array.from({ length: 9 }, (_, i) => categorical.tolMuted(i));
            const safety = accessibility.isPaletteSafe(colors);
            assert.isTrue(safety.protanopia, "Tol Muted should be safe for protanopia");
            assert.isTrue(safety.deuteranopia, "Tol Muted should be safe for deuteranopia");
        });
        ...
        it("Pastel palette is distinguishable", () => {
            ...
            assert.isTrue(safety.protanopia || safety.deuteranopia, "Pastel should be somewhat distinguishable");
        });
    });
    ...
    describe("Sequential gradients accessibility", () => {
        it("Viridis maintains perceptual uniformity", () => {
            ...
            for (let i = 0; i < samples.length - 1; i++) {
                ...
                assert.isAbove(diff, 10, `Adjacent viridis colors should be distinguishable (${i} to ${i + 1})`);
            }
        });
        ...
        it("Viridis gradient is safe for deuteranopia", () => {
            ...
            for (let i = 0; i < simulated.length - 1; i++) {
                ...
                assert.isAbove(diff, 5, `Viridis should remain distinguishable under deuteranopia (${i} to ${i + 1})`);
            }
        });
    ...
    describe("Diverging gradients accessibility", () => {
        it("Purple-Green avoids red-green confusion", () => {
            ...
            assert.isTrue(protanopiaSafe, "Purple-Green should be safe for protanopia (no red-green)");
        });

        it("Blue-Orange is distinguishable", () => {
            const samples = [0, 0.5, 1.0].map((v) => diverging.blueOrange(v, 0.5));
            const safety = accessibility.isPaletteSafe(samples);
            assert.isTrue(safety.protanopia || safety.deuteranopia, "Blue-Orange should be distinguishable");
        });

        it("Purple-Green is print-friendly", () => {
            const samples = [0, 0.5, 1.0].map((v) => diverging.purpleGreen(v, 0.5));
            const safety = accessibility.isPaletteSafe(samples);
            assert.isTrue(safety.grayscale, "Purple-Green should be distinguishable in grayscale (print-friendly)");
        });
    });
    ...
    describe("Comprehensive palette validation", () => {
        ...
        it("All sequential gradients maintain distinguishability", () => {
            ...
            gradients.forEach(({ name, fn }) => {
                ...
                for (let i = 0; i < samples.length - 1; i++) {
                    const diff = accessibility.colorDifference(samples[i], samples[i + 1]);
                    assert.isAbove(diff, 8, `${name} gradient should have distinguishable steps`);
                }
            });
        });
    });
