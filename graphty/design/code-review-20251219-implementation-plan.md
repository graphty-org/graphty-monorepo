...
- `src/hooks/__tests__/useLocalValue.test.ts`: Test the new local value hook
  ...
  describe('useLocalValue', () => {
    ...
    it('returns parsed numeric value on commit', () => {
      ...
      act(() => { result.current.setLocalValue("10.5"); });
    ...
    it('returns fallback for invalid input', () => {
      ...
      act(() => { result.current.setLocalValue("invalid"); });
...
1. `src/components/sidebar/edge-controls/EdgeLineControl.tsx`:
   ...
   - Before:
     ...
     useEffect(() => { setLocalWidth(value.width); }, [value.width]);
   ...
   - After:
     ...
     const { localValue: localWidth, setLocalValue: setLocalWidth, commitValue: commitWidth } =
...
*Based on: code-review-20251219.md*
