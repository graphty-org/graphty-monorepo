...
const WEBLLM_MODELS: WebLLMModel[] = [
    {id: "Llama-3.2-1B-Instruct-q4f32_1-MLC", name: "Llama 3.2 1B", size: "~700 MB", description: "Smallest, fastest"},
    {id: "Llama-3.2-3B-Instruct-q4f32_1-MLC", name: "Llama 3.2 3B", size: "~1.8 GB", description: "Good balance"},
    {id: "Phi-3.5-mini-instruct-q4f16_1-MLC", name: "Phi 3.5 Mini", size: "~2 GB", description: "Microsoft's compact model"},
...
export function AiSettingsModal({
    ...
    const handleClearKey = useCallback((provider: ProviderType) => {
                    ...
                    <>
                        ...
                        <Group gap="sm">
                            <Button
                                ...
                                leftSection={state.testStatus === "testing" ?
                    ...
                    <Stack gap="md">
                        ...
                        {webllmModel && (
                            <Alert icon={<Download size={16} />} color="yellow" variant="light">
                                <Text size="sm" fw={500}>Download Required</Text>
                                <Text size="xs" c="dimmed">
                                    The selected model ({WEBLLM_MODELS.find((m) => m.id === webllmModel)?.size ?? "unknown size"}) will be downloaded to your browser cache when first used.
        ...
        >
            <Stack gap="lg">
                ...
                <Select
                    ...
                    rightSection={hasKey(selectedProvider) || providerStates[selectedProvider]?.key ? (
                        ...
                        {persistenceEnabled && !isPersistenceEnabled && (
                            <>
                                <Alert icon={<ShieldAlert size={16} />} color="yellow" variant="light">
                                    <Text size="sm" fw={500}>Security Notice</Text>
                                    <Text size="xs">
                                        Browser storage is convenient but not fully secure. API keys could be accessed
                                        by browser extensions, XSS attacks, or anyone with access to this device.
                                        For sensitive keys, consider re-entering them each session instead.
                                    </Text>
                                </Alert>
