...
describe("Multipartite Layout", () => {
    ...
    describe("Special cases and edge cases", () => {
        ...
        it("should produce consistent results", () => {
            ...
            const subsets = [graph.nodes().slice(0, 5), graph.nodes().slice(5, 10), graph.nodes().slice(10, 15)];
        ...
        it("should handle complete k-partite graphs", () => {
            ...
            const avgX = subsets.map((subset) => subset.reduce((sum, n) => sum + positions[n][0], 0) / subset.length);
        ...
        it("should minimize edge crossings between adjacent layers", () => {
            ...
            const graph = {
                ...
                edges: () => [
                    ["A0", "B0"],
                    ["A1", "B1"],
                    ["A2", "B2"], // Parallel edges
                    ["B0", "C0"],
                    ["B1", "C1"],
                    ["B2", "C2"], // Parallel edges
                ],
            };
            const subsets = [
                ["A0", "A1", "A2"],
