 ...
 * ONE dispatch, chosen by `frontier-finalize` for a frontier below `P.fusedMax` entries (`SLOT.fused`) and for the
 * retry of a level whose edge queue overflowed (`SLOT.fusedRetry`, PD-23). One WORKGROUP per frontier entry (the
 * selector sizes both slots through `write_slot_groups`, never per invocation): lane 0 reads the entry's row clipped
