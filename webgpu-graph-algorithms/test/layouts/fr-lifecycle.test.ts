/**
 * Lifecycle of the Fruchterman-Reingold simulation (spec 11.3 "Device loss / errors / leaks"; P5-T4 Step 2, the
...
describe("FR lifecycle (spec 11.3)", () => {
    it("dispose() leaves no live buffer; a step() maps at most two staging slots (scene + state)", async (t) => {
        ...
        try {
            const sim = createFrSim(own, FR_BASE_OPTIONS, FR_TUNING);
            const positions = startPositions(s, FR_BASE_OPTIONS, false);
    ...
    it("release(snapshot) during a live simulation: the next step() rejects E_RELEASED; a load of another snapshot works; snapshots === 1", async (t) => {
        requireGpu(t);
        const ctx = await acquire({ label: "fr-lifecycle/release" });
        const first = paritySnapshot("karate", 1, false);
        const second = paritySnapshot("grid10", 1, false);
        try {
            await withFrSim(ctx, FR_BASE_OPTIONS, FR_TUNING, async (sim) => {
                sim.load(first, startPositions(first, FR_BASE_OPTIONS, false));
                await sim.step(2);
                ...
                sim.load(second, startPositions(second, FR_BASE_OPTIONS, false));
                await sim.step(2);
                expect(ctx.residency.stats().snapshots).toBe(1);
    ...
    it("device loss mid-run: the pending step rejects E_DEVICE_LOST, the simulation is disposed, the context is lost; a fresh context runs afterwards", async (t) => {
        requireGpu(t);
        const ctx = await acquire({ label: "fr-lifecycle/lost" });
        const s = paritySnapshot("random1k", 1, false);
        const sim = createFrSim(ctx, { ...FR_BASE_OPTIONS, iterations: 100_000 }, FR_TUNING);
        ...
        const fresh = await acquire({ label: "fr-lifecycle/recovered" });
        try {
            await withFrSim(fresh, FR_BASE_OPTIONS, FR_TUNING, async (recovered) => {
                const positions = startPositions(s, FR_BASE_OPTIONS, false);
